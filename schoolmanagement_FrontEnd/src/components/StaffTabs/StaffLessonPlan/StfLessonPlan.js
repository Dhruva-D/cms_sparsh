...
const StfLessonPlan = () => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState({}); // New state for file uploads
  ...
  // Custom Hooks with dependencies (matching Admin page exactly)
    ...
    const fetchMentors = async () => {
      try {
        ...
        if (data && data.data) {
          ...
          if (userId) {
            const loggedInTeacher = formattedMentors.find(
              (mentor) => mentor.value === parseInt(userId)
            );
            if (loggedInTeacher) {
              setSelectedMentor(loggedInTeacher);
            }
          }
        }
      } catch (error) {
    ...
    if (lessonPlanData.length > 0) {
      setUpdatedData(
        lessonPlanData.map((item) => ({
          ...
          document_file: item.document_file || null // Add document_file
        }))
      );
    } else {
      setUpdatedData([]);
  ...
  const handleUpdate = async (index) => {
    ...
    try {
      ...
      if (result.message === "success") {
        ...
        // Clear uploaded file for this row
        setUploadedFiles(prev => {
          const newState = { ...prev };
  ...
  const handleClear = () => {
    ...
    setLessonPlanData([]);
    setUpdatedData([]);
    // Note: selectedMentor is NOT cleared as it's auto-selected for logged-in teacher
  };
